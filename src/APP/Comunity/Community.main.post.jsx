...
function Post() {
  ...
  const [postResponse, setPostResponse] = useState([]); //게시글
    ...
    const fetchData = async () => {
      try {
        if (type === 'best') {
					const bestResponse = await request.get('/api/posts/search?page=0&size=5&sort=hits,desc');
					setBestResponse(bestResponse.data.content);
					console.log('bestResponse', bestResponse);
        } else if (type === 'storereview') {
					const storeReviewsResponse = await request.get('/api/reviews/all?type=store&page=0&size=5&sort=id,desc');
					...
					const productReviewsResponse = await request.get('/api/reviews/all?type=product&page=0&size=5&sort=id,desc');
					...
					const postResponse = await request.get('/api/posts/search?page=0&size=5&sort=id,desc');
