...
function IsViewing(props) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                console.log('아이디:', props.data[0].id);
                if (isSuccess) {
                    if (props.isProduct === true) {
                        const response = await request.get(`/api/reviews/auth?type=product&id=${props.data[0].id}`, {
                            headers: {
                        ...
                        const response = await request.get(`/api/reviews/auth?type=store&id=${props.data[0].id}`, {
                            headers: {
                                'Content-Type': 'multipart/form-data',
                                Authorization: `Bearer ${window.localStorage.getItem(ACCESS_TOKEN)}`
                            },
                    ...
                    if (props.isProduct === true) {
                        ...
                        const response = await request.get(`/api/reviews?type=product&id=${props.data[0].id}`);
                        setReview(response.data);
                    } else {
                        const response = await request.get(`/api/reviews?type=store&id=${props.data[0].id}`);
                        setReview(response.data);
                    }
                }
                if (props.isProduct === true) {
